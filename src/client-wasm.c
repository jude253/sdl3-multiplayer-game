...
NOTE: something is not working with this. It doesn't not seem to be able
to connect to the mongoose server.  I am speculating that it cannot by
default upgrade from http to ws as the tutorial server does.
*/
...
EMSCRIPTEN_WEBSOCKET_T create_socket() {
  ...
  const char *url = "ws://localhost:8000/websocket";
  attr.url = url;
